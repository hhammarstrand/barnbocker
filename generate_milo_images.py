...
from minimax_pipeline import MinimaxPipeline

BOOK_DIR = Path("books/fifi-milo-lost")
...
def main():
    ...
    for i, prompt_text in enumerate(data["imagePrompts"], 1):
        ...
        full_prompt = f"""Soft watercolor children's book illustration. Cute baby red fox, tiny brown mouse, and owl with spectacles. {prompt_text}. Warm colors, soft edges, no text in image."""

        if len(full_prompt) > 1500:
            full_prompt = full_prompt[:1490] + "."
